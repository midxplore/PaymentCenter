...
public class SysMenuService : IDynamicApiController, ITransient
{
    ...
    {
        var iSugarQueryable = _sysMenuRep.AsQueryable().Where(u => u.Status == StatusEnum.Enable).OrderBy(u => new { u.OrderNo, u.Id });

        if (_userManager.SuperAdmin)
        {
            var menuTree = await iSugarQueryable.Where(u => u.Type != MenuTypeEnum.Btn).ToTreeAsync(u => u.Children, u => u.Pid, 0);
            return menuTree.Adapt<List<MenuOutput>>();
        }
        else
        {
            var menuIdList = await GetMenuIdList();
            var menuTree = await iSugarQueryable.ToTreeAsync(u => u.Children, u => u.Pid, 0, menuIdList.Select(d => (object)d).ToArray());
    ...
    public async Task<List<SysMenu>> GetList([FromQuery] MenuInput input)
    {
        ...
        var iSugarQueryable = _sysMenuRep.AsQueryable().OrderBy(u => new { u.OrderNo, u.Id });
        return _userManager.SuperAdmin
            ? await iSugarQueryable.ToTreeAsync(u => u.Children, u => u.Pid, 0)
            : await iSugarQueryable.ToTreeAsync(u => u.Children, u => u.Pid, 0, menuIdList.Select(d => (object)d).ToArray());
    }
