...
public class SysRoleService : IDynamicApiController, ITransient
{
    ...
    public async Task AddRole(AddRoleInput input)
    {
        ...
        var nameList = new List<string>() { "dashboard", "home", "notice", "sysUserCenter" };
        var permissionList = new List<string>() { "sysUser/changePwd", "sysUser/baseInfo", "sysUser/uploadSignature", "sysUser/uploadAvatar" };
        var menuIdList = menuList.Where(u => nameList.Contains(u.Name) || permissionList.Contains(u.Permission)).Select(u => u.Id).ToList();
