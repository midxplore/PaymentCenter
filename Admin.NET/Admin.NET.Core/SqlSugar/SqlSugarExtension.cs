...
public static class SqlSugarExtension
{
    ...
    public static void InitTable(this SqlSugarScopeProvider dbProvider, Type entityType)
    {
        ...
        foreach (var dbColumnInfo in dbColumnInfos.Where(dbColumnInfo => !dbColumnInfo.IsPrimarykey && entityInfo.Columns.All(u => u.DbColumnName == null || !u.DbColumnName.Equals(dbColumnInfo.DbColumnName, StringComparison.CurrentCultureIgnoreCase))))
        {
    ...
    public static (int, int, int)? InitTableSeedData(this SqlSugarScopeProvider dbProvider, Type seedType, Action<object> handleBefore = null)
    {
        ...
        if (config.ConfigId.ToString() == SqlSugarConst.MainConfigId) // 默认库（有系统表特性、没有日志表和租户表特性）
        {
            ...
            {
                Console.WriteLine(string.Format("    忽略 {0,-58} ({1}) 原因: 非SysTable 与 (LogTable 或 Tenant)表", seedType.FullName, dbProvider.CurrentConnectionConfig.ConfigId));
                return default;
            }
        }
        else if (config.ConfigId.ToString() == SqlSugarConst.LogConfigId) // 日志库
        {
            ...
            {
                Console.WriteLine(string.Format("    忽略 {0,-58} ({1}) 原因: 非LogTable表", seedType.FullName, dbProvider.CurrentConnectionConfig.ConfigId));
                return default;
            }
        }
        else
        {
            ...
            {
                Console.WriteLine(string.Format("    忽略 {0,-58} ({1}) 原因: 非Tenant表", seedType.FullName, dbProvider.CurrentConnectionConfig.ConfigId));
                return default;
            }
        }
        ...
        {
            Console.WriteLine(string.Format("    忽略 {0,-58} ({1}) 原因: 没有种子数据", seedType.FullName, dbProvider.CurrentConnectionConfig.ConfigId));
            return default;
        }
