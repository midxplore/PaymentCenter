...
public sealed class SignatureAuthenticationHandler : AuthenticationHandler<SignatureAuthenticationOptions>
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        //
        // 字符串 != 会在**第一个不同字符**处短路返回，比较耗时随「前缀匹配长度」变化。
        // 远端理论上可以据此逐字节试出正确的签名值（timing oracle）。
        //
        // 对本项目而言实际可利用性很低（HMAC-SHA256 的 44 字符 base64 摘要，
        // 且网络抖动远大于逐字符差异），但这是**零成本**的正确写法：
        // 签名比较属于密码学比较，就该用密码学比较函数，不给审计留下「已知弱点」的条目。
        if (!FixedTimeEquals(serverSign, sign))
    ...
    /// <remarks>
    /// <para>
    /// 直接写 <c>a != b</c> 会在首个不同字符处短路，比较耗时与「公共前缀长度」相关，
    /// 构成一个 timing oracle。本方法改用 <see cref="CryptographicOperations.FixedTimeEquals"/>，
    /// 逐字节比较且不提前返回。
    /// </para>
    /// <para>
    /// 长度不等时立即返回 <c>false</c>：长度本身不是秘密（HMAC-SHA256 的 Base64 结果恒为 44 字符），
    /// 且 <see cref="CryptographicOperations.FixedTimeEquals"/> 在长度不同时本来也会立即返回 false。
    /// </para>
    /// </remarks>
