...
public static class SignatureAuthenticationDefaults
{
    ...
    /// <summary>
    /// 附加在 HttpContext Item 中「已通过签名校验的开放接口身份」的 Key
    /// </summary>
    /// <remarks>
    /// 值为 <see cref="SysOpenAccess"/>。业务层应从这里取调用方身份，
    /// 而不是自己去读 <c>accessKey</c> 请求头再反查一次——那等于把已经校验过的身份重新推导一遍。
    /// </remarks>
    public const string OpenAccessItemKey = "SignatureOpenAccess";
}