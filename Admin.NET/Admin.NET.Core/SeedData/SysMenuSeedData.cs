...
public class SysMenuSeedData : ISqlSugarEntitySeedData<SysMenu>
{
    ...
    public IEnumerable<SysMenu> HasData()
    {
        ...
        [
            ...
            // ★ 2026-09-17 补：createSecret / generateSignature 原先**没有挂任何按钮权限**。
            //   框架的鉴权是「黑名单」模型（SysRoleService.GetUserApiList 里 apiList[1] = 全部按钮 − 用户已有按钮，
            //   JwtHandler 据此拦截），于是**没映射到按钮的接口对任何已登录用户放行**。
            //   createSecret 只是随机串生成器、generateSignature 需要先把密钥当入参传进来，
            //   所以危害有限；但它们属于「资金类接口的凭证管理面」，必须纳入 RBAC 才谈得上完整审计。
            //
            // ★★ Permission 必须是**路由的尾部**（JwtHandler 按 path.EndsWith(permission) 匹配），
            //    而路由由方法名推导、不是方法名的原样：CreateSecret → `/api/sysOpenAccess/secret`。
            //    写成 "sysOpenAccess/createSecret" 会**永远匹配不上** → 权限形同虚设、接口仍对所有人开放。
            //    （GenerateSignature 恰好同名，所以那条是对的。）改这里时务必对照前端
            //    `Web/src/api-services/system/apis/sys-open-access-api.ts` 里的 localVarPath。
            new SysMenu{ Id=1310000000426, Pid=1310000000421, Title="生成密钥", Permission="sysOpenAccess/secret", Type=MenuTypeEnum.Btn, CreateTime=DateTime.Parse("2026-09-17 00:00:00"), OrderNo=100 },
            new SysMenu{ Id=1310000000427, Pid=1310000000421, Title="生成签名", Permission="sysOpenAccess/generateSignature", Type=MenuTypeEnum.Btn, CreateTime=DateTime.Parse("2026-09-17 00:00:00"), OrderNo=100 },
