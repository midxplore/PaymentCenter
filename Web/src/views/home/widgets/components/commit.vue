...
// ★ 必须显式导入：项目里没有把 axios 挂到全局（无 window.axios / globalProperties.$axios），
// 这里原本直接裸用 axios，运行时是 ReferenceError（本目录被 index.ts 的
// import.meta.glob('./*.vue', {eager:true}) 全部急加载，组件一挂载就会执行 getList）。
